...
#
# "100 180 90 56 65 74 68 86 99" When two numbers have the same "weight", let us class them as if they were strings (
...
#
# Notes it may happen that the input string have leading, trailing whitespaces and more than a unique whitespace
# between two consecutive numbers For C: The result is freed.

def order_weight(strng):
    array = sorted(strng.split())
    for i in range(len(array)):
        array[i] = sum([int(j) for j in array[i]])
    combined = zip(array, sorted(strng.split()))
    answ = sorted(combined, key=lambda x: x[0])
    return ' '.join([i[1] for i in answ])


a = order_weight("103 301 123 4444 99 2000")
print(a)